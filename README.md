...
- 非阻塞 Google Search grounding；結果會在目前發言結束後回報
- 30 種 Gemini 原生聲線與本機 API key 設定
...
5. 點擊工具列圖示開啟 side panel，選取網頁區塊或上傳檔案後開始對談。
...
- 字幕、音訊、grounding 工作與 session handle 只存在記憶體。
...
- 第一版不支援 Office、圖片／影音、OCR、多來源累加、歷史保存或自動摘要。
...
測試涵蓋來源處理、檔案解析、PDF 頁序、Live setup／`setupComplete` 探測、模型 allowlist、純文字模式、快捷鍵、串流字幕合併、非阻塞 grounding、`WHEN_IDLE` 回應、權限錯誤分類與 PCM 音訊轉換。
